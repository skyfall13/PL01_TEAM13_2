...
def run_func(op_code_node):
    ...
    def define(node):
        l_node = node.value.next
        r_node = l_node.next
        insertTable(l_node.value, run_expr(r_node))
    ...
    def run_cond(node):
        ...
        elif node.value.type is TokenType.LIST:
            if run_func(node.value.value)(node.value).type is TokenType.FALSE:
                return run_cond(node.next)
            else:
                return node.value.next
...
def print_node(node):
    ...
    if node.type in [TokenType.ID, TokenType.INT]:
        if node.value in global_val:
            return str(global_val[node.value])
        return node.value
